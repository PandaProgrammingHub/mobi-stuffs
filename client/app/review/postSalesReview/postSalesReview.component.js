...
import routes from './postSalesReview.routes';
...
export class PostSalesReviewComponent {
  /*@ngInject*/
  constructor($http, $scope, $state, Auth,mFactory, $window) {
    this.$window = $window;
    this.Auth = Auth;
    this.$http = $http;
    this.$state = $state;
  ...
  handlePostSalesSubmit() {
    ...
    currentUser.then(userdata=>{
      ...
      if(self.postSales.decisionTime === 'Decision Time') {
        self. postSales.decisionTime = '';
      }
  ...
  addPostSalesReview(postSales) {
    ...
    this.$http.post('/api/reviews', postSales).then(response=>{
      self.submitted = true;
      self.mFactory.setPostSale(true);
...
export default angular.module('salesdoorApp.postSalesReview', [uiRouter])
  .config(routes)
  .component('postSalesReview', {
    template: require('./postSalesReview.html'),
    controller: PostSalesReviewComponent,
    controllerAs: 'postSalesReviewCtrl'
  })
  .name;
